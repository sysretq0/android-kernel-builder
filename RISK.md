...
| ModuleGate LSM (audit) | MEDIUM | **Live incident**: the finit-module hook (`kernel_post_read_file`, previously dead code — legacy hook never fires for `finit_module`) bootloops 5.10 on-device per device report (~8s silent loop). Shipped fix: cached tfm (never allocs in steady state, no hook re-entrancy), quiet enroll-success (`pr_debug`, sysfs stays complete), release/acquire publish. **Dropped from `plain` pending device proof**; `susfs` variant still pins it |
