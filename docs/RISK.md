...
| ModuleGate LSM (audit) | MEDIUM | **Does not boot, shelved**: the finit-module hook bootloops 5.10 on-device per device report (~8s silent loop). A fix is written but unproven and untouched until further notice — dropped from both variants, no device time planned. Re-enable only with a booting build in hand |
